...
const titles: Record<string, string> = {
  'fetch-etfs': '定位产业关联 ETF', 'fetch-etf-data': '获取ETF行情及关键指标', 'fetch-etf-holdings': '解析 ETF 成分股持仓',
  'fetch-companies': '提取产业链企业节点', 'fetch-company-data': '采集企业行情数据', 'fetch-news': '采集产业资讯并汇总情绪',
  'fetch-market-snapshot': '分析市场指数与板块资金', 'calculate-market-trends': '计算 ETF 与企业趋势指标',
  'market-analysis': '解读 ETF 行情与资金趋势', 'news-analysis': '研判资讯事件与市场情绪', 'company-analysis': '分析产业链企业基本面',
  'industry-overview': '整合多维度行业总览', 'investment-advice': '生成综合评分与配置建议', 'generate-report': '汇编并保存综合分析报告',
}

const labels: Record<string, string> = {
  'etf-bindings': '关联 ETF', 'etf-market-data': 'ETF 行情数据', 'report-content': '分析报告',
  'report-id': '报告编号', companies: '产业链企业', 'company-codes': '企业代码',
  'company-market-data': '企业行情数据', news: '产业资讯', 'market-snapshot': '市场概览',
  'market-trends': '市场趋势', 'industry-info': '产业信息', 'etf-codes': 'ETF 代码',
  code: '代码', ticker: '代码', name: '名称', description: '描述', weight: '权重',
  bind_type: '绑定类型', bindType: '绑定类型', etf_code: 'ETF 代码', etf_name: 'ETF 名称',
...
function labelFor(key: string) {
  return labels[key] || labels[key.replace(/-([a-z])/g, (_, letter) => letter.toUpperCase())] || key.replace(/[-_]/g, ' ').replace(/(^|\s)\S/g, (letter) => letter.toUpperCase())
}
...
function SectorFlowSummary({ sectors }: { sectors: any[] }) {
  ...
  const inflow = rows.filter((item) => item.flow > 0).sort((a, b) => b.flow - a.flow).slice(0, 10)
  const outflow = rows.filter((item) => item.flow < 0).sort((a, b) => a.flow - b.flow).slice(0, 10)
...
function VisualArtifact({ artifact }: { artifact: Artifact }) {
  ...
  if (artifact.artifactKey === 'report-content' || artifact.artifactKey.endsWith('analysis') || artifact.artifactKey === 'industry-overview' || artifact.artifactKey === 'investment-advice') {
    return <div className="prose prose-sm max-w-none dark:prose-invert"><ReactMarkdown>{typeof data === 'string' ? data : pretty(data)}</ReactMarkdown></div>
  }
  if (Array.isArray(data)) {
    if (artifact.artifactKey === 'etf-market-data') {
      return <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{data.map((item: any, index) => <div key={item.code || item.ticker || index} className="rounded-xl border p-4"><div className="flex items-start justify-between gap-2"><div><div className="font-medium">{item.name || item.code || item.ticker || 'ETF'}</div><div className="text-xs text-muted-foreground">{item.code || item.ticker || '暂无代码'}</div></div><Badge variant="outline">{item.trend || '震荡'}</Badge></div><div className="mt-3 grid grid-cols-2 gap-3 text-xs"><Metric label="最新价格" value={item.price ?? item.current_price} /><Metric label="涨跌幅" value={item.change_pct ?? item.price_change_pct} /><Metric label="MA20" value={item.ma20} /><Metric label="RSI" value={item.rsi} /><Metric label="波动率" value={item.volatility} /><Metric label="最大回撤" value={item.max_drawdown} /></div></div>)}</div>
    }
...
export default function StepResultPage() {
  ...
  const visibleArtifacts = useMemo(
    () => (step?.artifacts || []).filter((artifact) => !['etf-codes', 'industry-info'].includes(artifact.artifactKey)),
    [step]
  )
  ...
  return <div className="space-y-6 p-6">
    ...
    <header className="flex flex-col gap-3 rounded-xl border bg-card p-5 sm:flex-row sm:items-start sm:justify-between">
      ...
      {reportId && <Button onClick={() => router.push(`/comprehensive-analysis/report/${reportId}`)}><FileText className="mr-2 h-4 w-4" />打开完整报告</Button>}
    </header>
