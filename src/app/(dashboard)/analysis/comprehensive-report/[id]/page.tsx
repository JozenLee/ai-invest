...
function MarketPanel({ market, marketSnapshot, etfs, indices }: { market: RecordValue; marketSnapshot: NormalizedMarket; etfs: unknown[]; indices: unknown[] }) {
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {samples.map((item, index) => {
            ...
            return <a key={display(item.symbol || item.id || item.name, index)} href={'#company-' + display(item.symbol || item.id || index)} className="rounded-xl border bg-muted/10 p-4 transition-colors hover:border-primary/50 hover:bg-muted/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
              <div className="flex items-start justify-between gap-3"><div><h3 className="font-semibold">{display(item.name, '未命名企业')}</h3><div className="mt-1 text-xs text-muted-foreground">{display(item.symbol, '暂无代码')}</div></div><Badge variant={display(signal.stance) === '积极观察' ? 'destructive' : 'outline'} className={stanceTone(signal.stance)}>{display(signal.stance, '暂不判断')}</Badge></div>
              <div className="mt-3 text-xs text-muted-foreground">{display(refs[0]?.segment_name, '未标注环节')}</div>
              <div className="mt-3 grid grid-cols-2 gap-3 border-y py-3 text-xs"><Metric label="最新财报期" value={display(financial.latest_period, '暂无')} /><Metric label="增长口径" value={display(financial.growth_basis, '无法确认')} /><Metric label="营收" value={formatAmount(financial.revenue)} /><Metric label="净利润" value={formatAmount(financial.net_profit)} /><Metric label="营收增速" value={formatPercent(financial.revenue_growth)} toneClass={tone(financial.revenue_growth)} /><Metric label="净利润增速" value={formatPercent(financial.profit_growth)} toneClass={tone(financial.profit_growth)} /></div>
              <div className="mt-3 text-xs leading-5"><div className="font-medium text-foreground">入选逻辑</div><p className="mt-1 line-clamp-4 text-muted-foreground">{normalizeSelectionReason(item.ai_selection_reason)}</p></div>
            </a>
          })}
        </div>
      </CardContent>
...
export default function ComprehensiveReportPage() {
  ...
  useEffect(() => {
    if (!params.id) return
    fetch(`/api/analysis/reports/${params.id}`)
      .then(async (response) => {
  ...
  const market = {
    ...
    index_analysis: normalizedMarket.indices,
  ...
  const indices = normalizedMarket.indices
  const news = normalizedNews.items
  const companies = normalizedCompany.topCompanies.slice(0, 12)
  ...
  const exportCurrentTab = async () => {
    ...
    try {
      ...
      const filename = `${report.industryName}-${tabLabel(activeTab)}-${new Date(report.createdAt).toISOString().slice(0, 10)}.jpg`
  ...
  return <main className="mx-auto max-w-7xl space-y-6 p-4 md:p-6">
    ...
    <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as TabKey)} className="space-y-5"><TabsList className="grid h-auto w-full grid-cols-2 gap-1 rounded-xl bg-muted/70 p-1 sm:grid-cols-3 lg:grid-cols-5"><TabsTrigger value="overview" className="min-h-10">总览</TabsTrigger><TabsTrigger value="market" className="min-h-10">市场</TabsTrigger><TabsTrigger value="news" className="min-h-10">资讯</TabsTrigger><TabsTrigger value="company" className="min-h-10">企业</TabsTrigger><TabsTrigger value="portfolio" className="min-h-10">持仓</TabsTrigger></TabsList>
      <TabsContent value="overview"><div ref={(node) => { tabRefs.current.overview = node }} className="min-w-0"><OverviewPanel report={report} advice={advice} recommendations={recommendations} quality={displayQuality} modules={modules} /></div></TabsContent>
      <TabsContent value="market"><div ref={(node) => { tabRefs.current.market = node }} className="min-w-0"><MarketPanel market={market} marketSnapshot={normalizedMarket} etfs={etfs} indices={indices} /></div></TabsContent>
      <TabsContent value="news"><div ref={(node) => { tabRefs.current.news = node }} className="min-w-0"><NewsPanel payload={newsPayload} news={news} /></div></TabsContent>
      <TabsContent value="company"><div ref={(node) => { tabRefs.current.company = node }} className="min-w-0"><CompanyPanel company={company} companies={companies} /></div></TabsContent>
      <TabsContent value="portfolio"><div ref={(node) => { tabRefs.current.portfolio = node }} className="min-w-0"><PortfolioPanel portfolio={portfolio} holdings={holdings} holdingValue={holdingValue} /></div></TabsContent>
    </Tabs>
  </main>
}
